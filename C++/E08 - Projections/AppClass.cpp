...
void Application::Display(void)
{
	...
	case 3:
		m_pCamera->ResetCamera();
		m_pCamera->SetUp(AXIS_Z * -0.5f);
		m_pCamera->SetPosition(AXIS_X * 29.5f);
		break;
	case 4:
		m_pCamera->ResetCamera();
		m_pCamera->SetPosition(AXIS_Z * -14.5f);
		break;
	case 5:
		m_pCamera->ResetCamera();
		m_pCamera->ResetCamera();
		m_pCamera->SetNearFar(vector2(4.5f, 900.0f));
		m_pCamera->SetPosition(AXIS_Z * -14.5f);
		break;
	case 6:
		m_pCamera->ResetCamera();
		m_pCamera->SetNearFar(vector2(0.001f, 9.5f));
		m_pCamera->SetPosition(AXIS_Z * -14.5f);
		break;
	case 7:
		m_pCamera->ResetCamera();
		m_pCamera->SetUp(AXIS_Y * -0.5f);
		break;
	case 8:
		m_pCamera->ResetCamera();
		m_pCamera->SetPerspective(true);
		m_pCamera->SetFOV(50.0f);
		m_pCamera->SetUp(AXIS_XY * -0.2f);
		break;
	case 9:
		m_pCamera->ResetCamera();
		m_pCamera->SetUp(AXIS_YZ * -0.6f);
		m_pCamera->SetHorizontalPlanes(vector2(-0.59f, 999.99f));
		m_pCamera->SetResolution(vector2(0.1f, 1.0f));
		break;
	case 0:
		m_pCamera->ResetCamera();
		m_pCamera->SetUp(AXIS_XZ * -0.9f);
		m_pCamera->SetFOV(-20.0f);
		m_pCamera->SetPerspective(true);
		break;
	}
