...
uint MyRigidBody::SAT(MyRigidBody* const a_pOther)
{
	...
	//create vector lists containing the local axes of the objects
	std::vector<vector3> localObj;
	std::vector<vector3> localOther;

	//add the axes to the vector lists
	localObj.push_back(GetModelMatrix()[0]);
	localObj.push_back(GetModelMatrix()[1]);
	localObj.push_back(GetModelMatrix()[2]);

	localOther.push_back(a_pOther->GetModelMatrix()[0]);
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			R[i][j] = glm::dot(localObj[i], localOther[j]);
		}
	}
	...
	t = vector3(glm::dot(t, localObj[0]), glm::dot(t, localObj[1]), glm::dot(t, localObj[2]));
...
bool MyRigidBody::IsColliding(MyRigidBody* const a_pOther)
{
	...
	*/
	if (bColliding) //they are colliding with bounding sphere
	{
		...
		//if the SAT does not return a 1, then the models are not colliding
		if (nResult != 1)
		{
			bColliding = false;
		}
