...
void MyCamera::MoveForward(float a_fDistance)
{
	...
	//calulate movement going forward and backwards using forward vector
...
void MyCamera::MoveVertical(float a_fDistance)
{
	...
	//calculate movement going up and down using upward vector
...
{
	//Tip:: Look at MoveForward
	//calculate movement going left and right using rightward vector
...
void MyCamera::CalculateView(void)
{
	...
	//initialize and calculate the values of the pitch and yaw
	//use the x value of pitchyawroll vector and the rightward directional vector to get the value for the pitch
	quaternion pitch = glm::angleAxis(m_v3PitchYawRoll.x, m_v3Rightward);
	//use the y value of pitchyawroll vector and the upward directional vector to get the value for the yaw
	quaternion yaw = glm::angleAxis(m_v3PitchYawRoll.y, m_v3Upward);

	//regulate speed of pitch, yaw and roll so the camera doesn't go haywire
	m_v3PitchYawRoll *= ZERO_V3;

	//update the forward vector by rotating the cross product of pitch and yaw 
	//as well as the forward vector
	m_v3Forward = glm::rotate(glm::cross(pitch, yaw), m_v3Forward);
	//add to the target variable
	m_v3Target = m_v3Forward + m_v3Position;

	//make it so that the camera will go left and right based on the current position of the camera
	m_v3Rightward = glm::cross(m_v3Forward, m_v3Upward);

	//make it so the camera can look upward
...
{
	//if the pitch is less than 89 degrees
	if (m_v3PitchYawRoll.x < 89)
	{
		m_v3PitchYawRoll.x += a_fDegree;
	}

	//if the pitch is greater than 89 degrees
	else
	{
		m_v3PitchYawRoll.x == 89;
	}
	
}
void MyCamera::ChangeYaw(float a_fDegree)
{
	//if the pitch is yaw than 89 degrees
	if (m_v3PitchYawRoll.y < 89)
	{
		m_v3PitchYawRoll.y += a_fDegree;
	}

	//if the yaw is greater than 89 degrees
	else
	{
		m_v3PitchYawRoll.y == 89;
	}
}
void MyCamera::ChangeRoll(float a_fDegree)
{
	//if the roll is less than 89 degrees
	if (m_v3PitchYawRoll.z < 89)
	{
		m_v3PitchYawRoll.z += a_fDegree;
	}

	//if the roll is greater than 89 degrees
	else
	{
		m_v3PitchYawRoll.z == 89;
	}
}
